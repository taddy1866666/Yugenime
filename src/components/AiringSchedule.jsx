...
const fetchSchedule = async (weekOffset) => {
  ...
  const weekStart = nowTimestamp + (weekOffset * 7 * 24 * 60 * 60);
  const start = weekStart - (15 * 24 * 60 * 60); // 15 days before the viewed week
  const end = weekStart + (15 * 24 * 60 * 60);   // 15 days after
  ...
  // Fallback: If no schedules found, fetch trending releasing anime
  if (airing.length === 0) {
    const fallbackQuery = `
      query {
        Page (perPage: 50) {
